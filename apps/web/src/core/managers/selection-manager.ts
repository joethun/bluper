...
import { expandToGroups } from "@/timeline/grouping";
...
export class SelectionManager {
	...
	constructor(private editor: EditorCore) {
		this.cleanupUnsub = editor.scenes.subscribe(() => {
			this.dropStaleSelection();
		});
	}

	/**
	 * Drops selected element and keyframe refs that no longer exist on the
	 * timeline. A command that deletes an element can move the selection forward
	 * (cut leaves the new right-hand piece selected), and a subsequent delete on
	 * the *other* piece clears the selection via the patch — but only if the
	 * user deletes the selected piece directly. Without this, a delete of one
	 * half of a split leaves the other half visibly highlighted (it is still in
	 * `selectedElements`), which is what the timeline was doing before.
	 */
	private dropStaleSelection(): void {
		...
		const liveElements: ElementRef[] = [];
		for (const ref of this.selectedElements) {
			const track = [...tracks.overlay, tracks.main, ...tracks.audio].find(
				(candidate) => candidate.id === ref.trackId,
			);
			if (!track) continue;
			if (!track.elements.some((element) => element.id === ref.elementId)) {
				continue;
			}
			liveElements.push(ref);
		}

		if (liveElements.length !== this.selectedElements.length) {
