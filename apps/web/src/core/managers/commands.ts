...
export class CommandManager {
	...
	/**
	 * Run `fn` with scene and timeline notifications coalesced, so a
	 * `BatchCommand` containing N sub-commands fires listeners once at the end
	 * instead of N times mid-loop. Without this, an undo/redo replays N
	 * `updateTracks()` calls → N scene notifies → N timeline notifies, which
	 * thrashes every subscribed React component.
	 */
	private batched<T>(fn: () => T): T {
		this.editor.scenes.beginChange();
		this.editor.timeline.beginChange();
		try {
			return fn();
		} finally {
			this.editor.timeline.endChange();
			this.editor.scenes.endChange();
		}
	}
