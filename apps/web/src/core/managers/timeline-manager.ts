...
export class TimelineManager {
	private changes = new ChangeNotifier();
	...
	beginChange(): void {
		this.changes.beginChange();
	}

	endChange(): void {
		this.changes.endChange();
	}
	...
	/**
	 * The empty span the given time falls in on that track, or `null` if it landed
	 * on material or past the end of it.
	 */
	findGapAtTime({
		trackId,
		time,
		...
		return findGapAtTime({ track, time });
	}

	/** Pulls everything after the gap back over it, on that track alone. */
	deleteGap({ gap }: { gap: TimelineGap }): void {
		const activeScene = this.editor.scenes.getActiveSceneOrNull();
		if (!activeScene) {
			return;
		}
		...
		const after = closeGap({ tracks: before, gap });
		if (after === before) {
			return;
		}
	...
	/** Closes every gap on one track in a single undo step. */
	deleteAllGaps({ trackId }: { trackId: string }): void {
		const activeScene = this.editor.scenes.getActiveSceneOrNull();
		const track = this.getTrackById({ trackId });
		if (!activeScene || !track) {
			return;
		}

		const before = activeScene.tracks;
		const after = closeAllGaps({ tracks: before, track });
		if (after === before) {
			return;
		}
