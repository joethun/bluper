...
function MaskItem({
	...
	return (
		<Section sectionKey={`mask-item:${mask.id}`} showTopBorder={false}>
			<SectionHeader
				trailing={
					// A fragment, not another flex wrapper: `SectionHeader` already lays
					// its trailing area out as a row, and the extra `gap-1` here spaced
					// these apart from the header actions in Effects and Adjust.
					<>
						<Button
							variant="ghost"
							size="icon"
			...
			>
				<div className="flex items-center gap-2">
					...
					{/* Same weight as every other section title — this one was
					    `font-normal`, so a mask read as lighter than the Effects,
					    Adjust and Graphic headers it sits alongside. */}
					<SectionTitle className="capitalize">{definition.name}</SectionTitle>
				</div>
			</SectionHeader>
			<SectionContent>
				<MaskParamsFields
				...
				<div className="flex flex-col gap-2">
					<div className="flex items-center gap-2">
						...
						<ColorPicker
							...
							sampleLabel="stroke color"
							onRequestSampleImage={requestSampleImage}
