...
class WasmCompositor {
	...
	/**
	 * Brings the cache in line with one renderer's frame.
	 *
	 * `owner` scopes retirement to the textures this renderer named. The cache is
	 * process-wide but renderers are not: a snapshot, a thumbnail or a freeze
	 * bake renders its own tree through the same compositor, and retiring every
	 * id its frame does not mention would take the preview's textures with it —
	 * closing `VideoFrame`s the preview's nodes still point at, which is a
	 * use-after-free the moment one is drawn.
	 */
	syncTextures({
		textures,
		owner,
		...
		const ownedPrefix = `${owner}:`;
		for (const previousId of this.cache.keys()) {
			if (!previousId.startsWith(ownedPrefix)) continue;
	...
	private syncVideoFrameTexture(texture: VideoFrameTextureDescriptor) {
		...
		// VideoFrame lifecycle belongs to the resolver: `resolveNode` in
		// `resolve.ts` closes the frame a node is replacing as it stores the new
		// one, so by the time a replacement reaches this cache its predecessor is
		// already released. Closing anything here would close a frame that is
		// either about to be uploaded or already gone.
	...
	private syncRenderedTexture(texture: RenderedTextureDescriptor) {
		...
		// Cleared in device pixels, drawn in the descriptor's own units: a
		// full-canvas texture below full render scale rasterises its canvas-space
		// drawing onto a smaller surface.
		ctx.setTransform(1, 0, 0, 1, 0, 0);
		ctx.clearRect(0, 0, texture.width, texture.height);
		const drawScale = texture.drawScale ?? 1;
		if (drawScale !== 1) {
