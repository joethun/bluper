...
function buildResampledBuffer({
	...
	const outputLength = Math.max(
		1,
		Math.ceil((clipDuration + preRoll) * targetSampleRate),
	);
	...
	for (let channel = 0; channel < numChannels; channel++) {
		...
		for (let i = 0; i < outputLength; i++) {
			// The buffer opens with `preRoll` seconds of the material before the
			// clip's own start, so the clip's first frame sits at `preRoll`.
			const clipTime = i / targetSampleRate - preRoll;
			const sourceTime =
				trimStart + getSourceTimeAtClipTime({ clipTime, clipDuration, retime });
...
async function buildPitchPreservedCurveBuffer({
	...
	const windowPlan = stretcherWindowPlan({
		clipDuration,
		targetSampleRate,
	});
	const {
	...
	const windowAverageRate = ({ index }: { index: number }) =>
		averageRateOverWindow({
			from: index * windowSeconds,
			to: Math.min(clipDuration, index * windowSeconds + windowSeconds),
			clipDuration,
			retime,
		});

	const shifter = new PitchShifter(stretchCtx, resampledBuffer, 4096);
	shifter.tempo = windowAverageRate({ index: 0 });
	shifter.pitch = 1;
	shifter.connect(stretchCtx.destination);
	...
	for (let index = 1; index < windowCount; index++) {
		...
		void stretchCtx
			...
			.then(() => {
				shifter.tempo = windowAverageRate({ index });
...
/**
 * A clip's audio, laid out ready to play.
 *
 * `preRollSeconds` is how much material before the clip's own start the buffer
 * opens with, which is not always what was asked for: the stretchers render a
 * span rather than sample a mapping, and have nowhere to put it. The caller has
 * to know which it got, because the number is also where the clip's first frame
 * sits in the buffer.
 */
...
export async function renderRetimedBuffer({
	...
	/**
	 * Seconds of the source before the clip's start to render ahead of it, for a
	 * caller that crossfades across the clip's edge. Honoured only where the clip
	 * is built by sampling the source; see {@link RenderedClipAudio}.
	 */
	preRoll?: number;
	...
	if (retime && hasRetimeCurve({ retime }) && maintainPitch) {
		...
		return {
			buffer: await buildPitchPreservedBuffer({
				sourceBuffer,
				trimStart,
				clipDuration,
				rate,
	...
	const safePreRoll = Number.isFinite(preRoll) ? Math.max(0, preRoll) : 0;
	return {
		buffer: buildResampledBuffer({
			audioContext,
			sourceBuffer,
			trimStart,
			clipDuration,
