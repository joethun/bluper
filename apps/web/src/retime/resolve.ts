...
/**
 * The speed the clip is playing at `clipTime`. Constant for a uniform rate; for
 * a curve it is the height of the curve at that point in the clip.
 */
export function getEffectiveRateAt({
	...
	if (
		...
		return getCurveAverageRate({ curve });
	}

	return getCurveRateAtPosition({
		curve,
		position: getCurveSourceFractionAtClipFraction({
			curve,
			clipFraction: Math.min(1, Math.max(0, clipTime / clipDuration)),
		}),
	});
}
