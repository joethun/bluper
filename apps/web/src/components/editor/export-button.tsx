...
/**
 * The codecs on offer for a container, learned by asking the engine rather than
 * from a table: the same build of the app runs on WebView2, WebKitGTK and every
 * browser, and they do not agree on what they can encode.
 */
function useEncodableVideoCodecs({
	...
	useEffect(() => {
		...
		let cancelled = false;
		void listEncodableVideoCodecs({ format, width, height }).then(
			(available) => {
...
function ExportPopover({
	...
	const availableCodecs = useEncodableVideoCodecs({
		format,
	...
	const handleExport = async () => {
		...
		if (result.success && result.artifact) {
			...
			if (result.artifact.kind === "opfs") {
			...
			} else if (result.artifact.kind === "path") {
				// The render already wrote a finished file. All that's left is
				// asking where it should live and moving it there — no bytes
				// pass through the page, whatever the export weighs.
				...
				await downloadBlob({ blob: result.artifact.blob, filename });
			}
