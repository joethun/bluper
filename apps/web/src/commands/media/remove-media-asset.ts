...
import { clearImageSource } from "@/services/renderer/nodes/image-node";
...
export class RemoveMediaAssetCommand extends Command {
	...
	execute(): CommandResult | undefined {
		...
		if (this.removedAsset.url) {
			clearImageSource({ url: this.removedAsset.url });
		}
	...
	undo(): void {
		...
		if (this.savedAssets && this.removedAsset) {
			// A desktop asset is read from a path and already carries a URL that
			// stays valid; only an in-memory `File` needs a fresh object URL,
			// because the one it had was revoked when the asset was removed.
