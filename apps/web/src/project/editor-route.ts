 ...
 *
 * The server-rendered build routes on the path — `/editor/<uuid>` — which is
 * what the URL should look like. The static export can't: a project id is a
 * runtime UUID, and `output: "export"` only emits pages for the params
 * `generateStaticParams` knows at build time. That's not just a deep-link
 * problem — the App Router fetches an RSC payload for *every* navigation,
 * including a click from the projects list, so a route with no prerendered
 * page can't be reached at all.
 *
 * So the export ships one shell page and carries the id in the query string.
 * Both builds go through {@link editorHref} and {@link readEditorProjectId},
 * and neither knows which one it is beyond the flag below.
 */
...
/**
 * True in the statically exported build (the desktop shell). Set by
 * `build:web:export`; absent everywhere else, including `next dev`.
 */
...
export function editorHref({ projectId }: { projectId: string }): string {
	if (!IS_STATIC_EXPORT) return `${EDITOR_PATH_PREFIX}/${projectId}`;
