...
/**
 * The element-level param shape. The Rust registry (see
 * `rust/crates/editor-core/src/params/registry.rs`) once carried a
 * `read`/`write` pair of closures per definition, for a value stored
 * somewhere other than the param bag. Nothing ever supplied them — every
 * built-in param lives in `element.params` — so they are gone rather than
 * ported. The Rust `read_element_param_value` and the matching TS code below
 * both fall back to the param's own default for the same reason.
 */
export type ElementParamDefinition<TKey extends string = string> =
	ParamDefinition<TKey>;

export { buildDefaultParamValues } from "@/wasm/params";
