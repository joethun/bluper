...
interface PreviewOptions {
	isMaskMode: () => boolean;
	/** Opens a clip for direct-manipulation cropping; see the crop overlay. */
...
export class PreviewInteractionController {
	...
	onDoubleClick({ clientX, clientY }: ReactMouseEvent): void {
		...
		// Footage and stills open for cropping, the way a slide editor does; text
		// opens for typing. Both are "get inside this element", which is what a
		// double click means here.
		if (hit.element.type === "video" || hit.element.type === "image") {
			this.deps.preview.enterCropMode({
				element: { trackId: hit.trackId, elementId: hit.elementId },
			});
			return;
		}
