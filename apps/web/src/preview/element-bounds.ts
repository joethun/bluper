...
import { getCropPlacement, readCropFromParams, resolveCropRect } from "@/crop";
...
function getVisualElementBounds({
	...
	/**
	 * The kept region of the source, when the clip is cropped. Mirrors
	 * `computeVisualTransform` in the frame descriptor: the box is fitted from
	 * the whole frame and the crop takes a sub-rectangle of it, so the handles
	 * sit round the picture that is actually drawn.
	 */
	...
	const localOffsetX = scaledWidth * centerFractionX;
	const localOffsetY = scaledHeight * centerFractionY;
	...
	return {
		...
		cy:
			...
			localOffsetX * sin +
			localOffsetY * cos,
		width: scaledWidth * keptFractionX,
		height: scaledHeight * keptFractionY,
...
/**
 * The whole frame's box on the canvas, ignoring any crop — the box the crop
 * overlay dims and drags inside. Only footage and stills have one; everything
 * else has no source edges to trim, so there is nothing to return.
 */
export function getUncroppedMediaBounds({
	element,
	canvasSize,
	mediaAsset,
	localTime,
	...
	mediaAsset?: MediaAsset | null;
	localTime: number;
}): { bounds: ElementBounds; sourceWidth: number; sourceHeight: number } | null {
	if (element.type !== "video" && element.type !== "image") {
		return null;
	}
