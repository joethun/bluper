...
function RenderTreeController() {
	...
	const croppingElementId = useCropModeStore(
		(s) => s.croppingElement?.elementId ?? null,
	);
	...
	useDeepCompareEffect(() => {
		...
		const renderTree = buildScene({
			...
			isPreview: true,
			uncroppedElementId: croppingElementId,
...
function PreviewCanvas({
	...
	const render = useCallback(async () => {
		...
		const frame = Math.floor(currentTime / ticksPerFrame);
		// Draw the frame the playhead is inside, not the instant it is on.
		//
		// Playback drives the clock from audio, so `currentTime` is an arbitrary
		// tick rather than a frame boundary. Rendering that instant literally means
		// a sub-frame gap between two clips — a couple of ticks left by a trim or a
		// drag — is a moment where neither clip is on, and the clear colour shows
		// through as a black flash at the join. Worse, the dedupe below is keyed on
		// the frame index, so that one unlucky instant is held for the whole frame.
		//
		// Quantising here also makes the preview agree with the export, which has
		// always rendered on the frame grid.
		const renderTime = frame * ticksPerFrame;
