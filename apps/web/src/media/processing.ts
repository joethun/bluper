...
import { probeMediaFile } from "./mediabunny";
import type { MediaProbe } from "./mediabunny";
...
async function readTimedMediaDetails({
	...
	if (result.status === "unreadable") {
		...
		// decoders rather than WebCodecs, and reads containers mediabunny won't —
		// which is the same fallback playback and export already rely on.
		...
		// Frames only ever come from mediabunny, so a container it refuses is one
		// the timeline could only show as nothing at all.
		toast.error(`Can't read ${file.name}`, {
