...
interface ResizeSession {
	...
	/**
	 * Snap candidates, built lazily on first use and reused for the gesture.
	 * The timeline cannot be edited mid-resize, so element edges and animation
	 * keyframes are fixed — rebuilding them per mousemove walked every element
	 * and every keyframe in the scene and threw the result away.
	 */
	snapPoints: SnapPoint[] | null;
	/**
	 * Playhead time captured at session start. The user does not move the
	 * playhead during a resize gesture, so reading it once is safe and lets
	 * `getSnapPoints()` produce a stable list without re-fetching per move.
	 */
...
export class ResizeController {
	...
	private snappedDelta({
		...
		const snapPoints = this.getSnapPoints({ session });
	...
	/**
	 * Snap candidates for the active gesture, built at most once. Snap point
	 * inputs are read once at session start (see `playheadTime` and `members`)
	 * and the timeline cannot be edited mid-resize, so a stable cache is safe.
	 */
	private getSnapPoints({ session }: { session: ResizeSession }): SnapPoint[] {
		if (session.snapPoints) return session.snapPoints;

		const snapPoints = buildElementGestureSnapPoints({
			tracks: this.config.getSceneTracks(),
			playheadTime: session.playheadTime,
			excludeElementIds: new Set(session.members.map((m) => m.elementId)),
		});
		session.snapPoints = snapPoints;
		return snapPoints;
	}
