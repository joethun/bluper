...
import { computeDropTarget } from "@/wasm/drop-target";
...
interface DragProgress {
	 ...
	 * The timeline cannot be edited mid-drag, so element edges and keyframes
	 * are fixed — rebuilding them per mousemove walked every element and every
	 * keyframe in the scene and threw the result away. Left null while snapping
	 * is off or shift is held, so those drags never build the list at all.
	 */
	snapPoints: SnapPoint[] | null;
	/**
	 * Playhead time captured at gesture start. The user cannot move the
	 * playhead mid-drag, so reading it once keeps the lazily-built snap list
